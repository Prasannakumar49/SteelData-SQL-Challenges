CREATE SCHEMA esports_tornament;

USE esports_tornament;
CREATE TABLE Teams (
...
SELECT teams.team_name,COUNT(players.player_id) AS No_of_players
FROM teams
INNER JOIN players ON teams.team_id = players.team_id
GROUP BY teams.team_name
ORDER BY COUNT(players.player_id);


#3. What is the total number of players in each team?
SELECT teams.team_name,COUNT(players.team_id) AS No_of_players 
...
SELECT teams.team_name, COUNT(winner_id) AS No_of_matches
FROM teams
INNER JOIN matches ON teams.team_id = matches.winner_id
GROUP BY teams.team_name;

#7. What is the average salary of players in the teams with country 'USA'?
SELECT p.player_name, AVG(p.salary) AS Average_salary
FROM players P INNER JOIN teams T ON P.team_id = T.team_id
WHERE T.country = 'USA';


#8. Which team won the most matches?
SELECT teams.team_name, MAX(matches.winner_id) AS Max_matches
...
SELECT matches.match_date,matches.score_team1,matches.score_team2
FROM matches
